...
public class GameHub : Hub
{
        ...
        QuizCategory category = QuizCategory.All)
    {
        ...
        var room = _rooms.CreateRoom(Context.ConnectionId, nickname, GameMode.Battle,
            rounds, timer, category);
        await Groups.AddToGroupAsync(Context.ConnectionId, room.Code);
        await Clients.Caller.SendAsync("RoomCreated", _rooms.ToRoomInfo(room));
        _log.LogInformation("Room {Code} created by {Nick} | Category: {Cat}", room.Code, nickname, category);
    }
    ...
    private async Task StartGame(string code)
    {
        ...
        // НОВОЕ: передаём категорию комнаты при выборе вопросов
        room.Questions = QuestionBank.GetRandomSet(room.MaxRounds, room.Category);
        room.CurrentRound = 0;
    ...
    private async Task SendNextQuestion(string code)
    {
        ...
        var dto = new QuestionDto
        {
            ...
            Text = q.Text,
            Category = q.Category,
            CategoryEnum = q.CategoryEnum,
            Options = q.Options,
            RoundNumber = room.CurrentRound + 1,
            TotalRounds = room.MaxRounds
        };

        // Сброс ответов и флага оценки
        foreach (var p in room.Players) { p.LastAnswerIndex = null; p.AnswerTimeMs = null; }
        room.IsEvaluating = false; // НОВОЕ: сбрасываем флаг перед каждым вопросом
    ...
    private async Task RunTimer(string code, int seconds, CancellationToken ct)
    {
        ...
        {
            if (ct.IsCancellationRequested) return; // ИСПРАВЛЕНИЕ: проверка в начале
            await Clients.Group(code).SendAsync(HubEvents.TimerTick, i, ct);
            await Task.Delay(1000, CancellationToken.None);
            if (ct.IsCancellationRequested) return; // ИСПРАВЛЕНИЕ: проверка перед EvaluateRound
        }

        // ИСПРАВЛЕНИЕ: двойная проверка — и токен, и флаг IsEvaluating
    ...
    private async Task EvaluateRound(string code)
    {
        ...
        // ИСПРАВЛЕНИЕ: защита от двойного вызова через флаг
        lock (room)
        {
        ...
        foreach (var p in room.Players)
        {
            ...
            int earned = _score.Calculate(correct, p.AnswerTimeMs ?? room.TimerSeconds * 1000L,
                room.TimerSeconds, p.Streak);
            p.Score += earned;
